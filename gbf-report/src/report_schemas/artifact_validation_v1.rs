...
use gbf_policy::{
    CompatibilityAdapterId, CompilerFeature, DiagnosticSeverity, EvidenceRef, FieldPath,
    RuntimeMode, ValidationCode, ValidationDetail, ValidationDiagnostic, ValidationOrigin,
};
...
fn semantic_error(name: &'static str, _value: impl Into<String>) -> ValidationDiagnostic {
...
mod tests {
    ...
    fn f_b2_artifact_validation_v1_self_hash_round_trip() {
        ...
        let mut env = ReportEnvelope::new(ReportOutcome::Passed, body).expect("envelope");
    ...
    fn f_b2_artifact_validation_v1_rejects_unknown_fields() {
        let mut env = ReportEnvelope::new(ReportOutcome::Passed, passing_body()).expect("envelope");
        env.report_self_hash = compute_self_hash(&env).expect("self hash");

...
use gbf_oracle::artifact::{
    ARTIFACT_ORACLE_LOG_TARGET, ArtifactOracle, ArtifactOracleInputs,
    EVENT_NAME_EVALUATION_COMPLETE, EVENT_NAME_EVALUATION_STARTED, EVENT_NAME_OBSERVATION_CAPTURED,
    EVENT_NAME_WEIGHT_RESOLVED, RealArtifactOracle,
};
use serde::Serialize;
...
#[test]
fn artifact_oracle_logging_emits_required_events() {
    ...
    assert_eq!(started.target, ARTIFACT_ORACLE_LOG_TARGET);
    ...
    assert_eq!(
        started.fields.get("artifact_self_hash"),
        Some(&artifact.artifact_self_hash.to_string())
    );

    let resolved = events
        .iter()
        .filter(|event| {
            event.fields.get("event_name").map(String::as_str) == Some(EVENT_NAME_WEIGHT_RESOLVED)
        })
        .collect::<Vec<_>>();
    assert_eq!(resolved.len(), product.weight_resolution_log.len());
    assert!(resolved.iter().all(|event| event.level == "TRACE"));
    assert!(resolved.iter().all(|event| {
        event.fields.get("resolved_via").map(String::as_str) == Some("QuantSpec::weight_quant")
    }));
    ...
    let expected_observation_count = product.observations.len().to_string();
    let expected_resolution_count = product.weight_resolution_log.len().to_string();
    assert_eq!(complete.level, "INFO");
    assert_eq!(
        complete.fields.get("observation_count").map(String::as_str),
        Some(expected_observation_count.as_str())
    );
    assert_eq!(
    ...
    assert_eq!(
        complete
            .fields
            .get("tensors_resolved_via_quant_spec")
            .map(String::as_str),
        Some(expected_resolution_count.as_str())
    );
    assert_eq!(
        complete
            .fields
            .get("tensors_resolved_via_naming")
            .map(String::as_str),
        Some("0")
    );
}
...
fn write_capture_if_requested(events: &[CapturedEvent]) {
    let Ok(path) = std::env::var("S3_ARTIFACT_CAPTURE_EVENTS") else {
        return;
    };
    ...
    std::fs::write(path, lines).expect("writes artifact events");
}

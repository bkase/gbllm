...
pub fn write_observations_if_requested(bytes: &[u8]) {
    if let Ok(path) = std::env::var("S3_DENOTATIONAL_OBSERVATIONS_OUT") {
        std::fs::write(path, bytes).expect("writes denotational observations");
    }
}

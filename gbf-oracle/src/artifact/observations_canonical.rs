//! Canonical encoding for sorted artifact observations.
...
use super::{
    ArtifactDeterminismClass, ArtifactObservations, Observation, OracleError, SemanticCheckpoint,
    WeightResolutionEntry,
};

const OBSERVATIONS_SCHEMA: &str = "artifact_observations.canonical.v1";

/// Encoder for `ArtifactObservations`.
#[derive(Debug, Clone, Copy)]
pub struct ArtifactObservationsCanonical;

impl ArtifactObservationsCanonical {
    /// Encode observations as S1 canonical JSON in sorted-key row order.
    pub fn to_vec(observations: &ArtifactObservations) -> Result<Vec<u8>, OracleError> {
    ...
    #[must_use]
    pub fn rows(observations: &ArtifactObservations) -> Vec<ObservationRow> {
...
impl ProductHashPayload {
    pub(crate) fn from_observations(
        ...
        Self {
            schema: "artifact_oracle_product_hash.v1",
            determinism_class,
            observations: ArtifactObservationsCanonical::rows(observations),
            weight_resolution_log: weight_resolution_log.to_vec(),
        }
    }
}
...
impl ObservationsPayload {
    fn from_observations(observations: &ArtifactObservations) -> Self {
        Self {

...
**2. Quality-first UX budget (bkase, 2026-07-04; revised same day to 30 s/char).** Generation as slow as **~30 seconds per character is acceptable**; the goal is the smartest model that fits, full stop — distillation from a large dense teacher is explicitly authorized as a primary quality lever. At 70% CPU that is ~22M M-cycles/token ≈ **~4.1M MACs/token** under weights-as-code (V3, 5.4 cy/MAC) or **~2.1M MACs/token** under the dispatch data-kernel (V2, 10.3 cy/MAC, 0.25 B/weight). At this budget the regime flips: cycles stop binding and **ROM capacity rules**, which favors V2-dispatch storage (a ~2.1M-param dense model needs only ~0.5 MiB as dispatch data) and above all **top-1 MoE with dispatch-encoded experts, which can store tens of millions of ternary parameters inside 8 MiB while spending ~2M MACs/token**. The S7 dense-vs-MoE verdict was pinned at matched deployed **bytes**; the decision-relevant comparison is matched **cycles/token** under the ROM ceiling, searched across dense-V3 / dense-V2 / MoE-V2 / mixed lowerings (bd-3771m).

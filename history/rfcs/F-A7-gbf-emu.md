...
1. A `gameroy-core`-backed `Emulator` value with a small, deliberate library API: `load_rom`, `step`, `run_for(budget)`, `run_frame()`, `run_until_pc(pc, budget)`, `run_fast_for(budget)`, `run_fast_frame()`, `run_fast_until_pc(pc, budget)`, register read/write, **bus** vs **debugger** memory access (`bus_read`/`bus_write` vs `peek`/`poke`), snapshot/restore, framebuffer, joypad injection.
...
- **Cycle units are typed.** F-A7 introduces `ClockCycles(u64)` and `MCycles(u64)` newtypes plus a `CycleBudget` enum. `run_for(budget)`, `run_until_pc(pc, budget)`, `run_fast_for(budget)`, and `run_fast_until_pc(pc, budget)` take `CycleBudget`; cycle-bearing fields in events / outcomes / lineage carry `ClockCycles`. The conversion `From<MCycles> for ClockCycles` is saturating `n * 4`. `DMG_FRAME_CLOCK_CYCLES = ClockCycles(154 * 456)` is the named one-frame budget used by `run_frame()`, `run_fast_frame()`, and long-run test budgets via `ClockCycles::saturating_mul`. The adapter's source-of-truth clock is whatever gameroy-core exposes (clock-count-oriented per upstream README); M-cycle views are derived.
...
- The `Emulator` API exposes a small fixed verb set: `load_rom(bytes, EmulatorConfig)`, `step`, `run_for(budget)`, `run_frame()`, `run_until_pc(pc, budget)`, `run_fast_for(budget)`, `run_fast_frame()`, `run_fast_until_pc(pc, budget)`, register access, **bus** vs **debugger** memory access (`bus_read` / `bus_write` / `peek` / `poke` / `peek_range`), snapshot/restore, framebuffer, joypad, harness polling. No `run_forever`, no thread spawn, no callback registration outside the trap dispatcher.
- The fast-run verbs are the explicit bulk API for long-running tests and benches that need to execute faster than real time. They loop gameroy-core's public interpreter primitive with lower adapter overhead, honor PC traps / `run_fast_until_pc` target stops / CPU idle / budget expiry, and suppress guest memory trace ingestion while no memory traps are installed. If memory traps are installed, they fail immediately with `EmuError::FastRunBlockedByMemoryTraps { memory_trap_count }` instead of silently slowing down; the caller must decide whether to remove the memory traps or call the fully instrumented runner.
...
- PC traps fire **before** executing the instruction at the registered address. `run_until_pc(pc, budget)` and `run_fast_until_pc(pc, budget)` return immediately if the emulator is already positioned at `pc`. Memory traps fire on the matching access; trace events and cycle counters update after the instruction.
...
- The bring-up `examples/load_tiny_rom.rs` consumes a **checked-in golden fixture** at `gbf-emu/tests/fixtures/tiny_rom.gb` (with provenance metadata and a regeneration command). `cargo test -p gbf-emu` does not shell out to `cargo run -p gbf-asm`. The boot test runs from an explicit `BootMode` (post-boot DMG state, or a pinned boot ROM) and asserts `run_fast_until_pc(0x0150, CycleBudget::Clock(DMG_FRAME_CLOCK_CYCLES.saturating_mul(100)))` reaches `$0150` — closing F-A1's deferred `tiny_rom_boots` at long last.
...
gbf-emu/src/
  ...
  adapter.rs              │  T-A7.1 + T-A7.2: gameroy-core wrapping (exact type pinned by spike);
                          │                   load_rom; step; run_for/run_frame; run_until_pc;
                          │                   run_fast_for/run_fast_frame/run_fast_until_pc; framebuffer; joypad
  primitives.rs           │  T-A7.2: Regs, MemAccess, Snapshot, Framebuffer, JoypadFrame, BankSnapshot,
                          │          ClockCycles/MCycles/CycleBudget, DMG_FRAME_CLOCK_CYCLES
...
The new modules add roughly 1,300 LOC of production code plus about 1.7 KLOC of fixture-driven and behavior tests, including the fast-run bulk API gates. The crate ships exactly one example (`examples/load_tiny_rom.rs`) — its acceptance is gated on a `cargo test -p gbf-emu` matrix plus the F-A1 follow-up `tiny_rom_boots` test now living here.
...
2. **Fast/bulk execution is an explicit API tier.** `run_for` / `run_frame` remain the fully instrumented bounded runners. `run_fast_for` / `run_fast_frame` / `run_fast_until_pc` are for long-running test and bench workloads that need to run faster than real time: they still take `CycleBudget`, still honor PC traps, target-PC stops, idle states, and budget expiry, but they skip guest memory trace ingestion unless memory traps are installed. If memory traps are present, the adapter returns `EmuError::FastRunBlockedByMemoryTraps { memory_trap_count }` without advancing execution, so callers choose intentionally between removing traps for speed or using the slow instrumented runner for watchpoint semantics.
...
- `Emulator::run_frame(&mut self) -> Result<RunOutcome, EmuError>` — shorthand for `run_for(CycleBudget::Clock(DMG_FRAME_CLOCK_CYCLES))`, using the DMG one-frame budget (`154 * 456` clock cycles) as a named constant instead of retyping the magic number.
...
- `Emulator::run_fast_for(&mut self, budget: CycleBudget)`, `Emulator::run_fast_frame(&mut self)`, and `Emulator::run_fast_until_pc(&mut self, pc: u16, budget: CycleBudget)` — bounded bulk runners for long-running tests and benches. These use the same `RunOutcome` shape and budget units as the fully instrumented runners, honor PC traps / target-PC stops / idle / budget expiry, suppress guest memory trace ingestion when no memory traps are installed, and return `EmuError::FastRunBlockedByMemoryTraps { memory_trap_count }` without advancing execution whenever memory traps exist.
- Typed cycle units: `ClockCycles(u64)`, `MCycles(u64)`, `CycleBudget` enum with `Clock(ClockCycles)` / `Machine(MCycles)`. `From<MCycles> for ClockCycles` is saturating `n * 4`; `ClockCycles::saturating_mul` supports multi-frame budgets without overflow. `DMG_FRAME_CLOCK_CYCLES = ClockCycles(154 * 456)` is exported. The adapter's source-of-truth clock is gameroy-core's clock count (per upstream README); M-cycle views are derived. `Emulator::clock_count() -> ClockCycles` and `Emulator::m_cycle_count_floor() -> MCycles` expose both.
...
- An `examples/load_tiny_rom.rs` plus a sibling `tests/tiny_rom_boots.rs` that close F-A1's deferred `tiny_rom_boots` test by calling `run_fast_until_pc(0x0150, MAX_TINY_ROM_BOOT_BUDGET)` from an explicit `BootMode`. The budget is `CycleBudget::Clock(DMG_FRAME_CLOCK_CYCLES.saturating_mul(100))`. The ROM bytes come from a checked-in golden fixture at `gbf-emu/tests/fixtures/tiny_rom.gb`; `cargo test` does not shell out to `cargo run -p gbf-asm`.
...
3. Calls `Emulator::run_fast_until_pc(0x0150, MAX_TINY_ROM_BOOT_BUDGET)?`. `MAX_TINY_ROM_BOOT_BUDGET` is `CycleBudget::Clock(DMG_FRAME_CLOCK_CYCLES.saturating_mul(100))`, giving the F-A1 100-frame intent a named DMG-clock-unit budget.
...
|------------------|-----------------------------------------------------------------------------------------------|---------------|
| `adapter.rs`     | `Emulator`, `EmulatorBuilder`, `EmulatorConfig`, `BootMode`, gameroy-core wrapping, `load_rom`, `step`, `run_for`, `run_frame`, `run_until_pc`, `run_fast_for`, `run_fast_frame`, `run_fast_until_pc`, bus/peek register/memory access, `clock_count`, `m_cycle_count_floor` | ~34 items |
| `primitives.rs`  | `Regs`, `Flags`, `Framebuffer`, `JoypadFrame`, `Snapshot`, `SnapshotLineage`, `EmuVersionTag`, `GitSha`, `BankSnapshot`, `RunOutcome`, `StepOutcome`, `CpuIdleState`, `MemoryAccess`, `MemoryAccessKind`, `ClockCycles`, `MCycles`, `CycleBudget`, `DMG_FRAME_CLOCK_CYCLES` | ~30 items |
...
Total public surface: ~100 items. Smaller than F-A2 (~160) because F-A7's job is to wrap *one* external crate in a typed adapter, not to enumerate hardware constants.
...
3. **Stepping / interpreter / bulk execution.** The exact path to advance one instruction, whether the core exposes any public frame-or-budget runner, and how to read the resulting clock count. If the pinned revision exposes only an instruction primitive, F-A7's bounded runners loop that primitive directly; no implementation may invent an unbounded `run_forever` surface.
...
11. **Fast bulk execution loops `Interpreter::interpret_op`; no public core frame runner is assumed.** The pinned `gameroy-core` revision exposes the public interpreter step primitive used by gameroy's own runners; any higher-level debugger/frontend loops live outside the headless core surface F-A7 can depend on. `run_fast_for` / `run_fast_frame` / `run_fast_until_pc` therefore loop `Interpreter::interpret_op()` with lower adapter overhead. They clear `io_trace` instead of ingesting guest memory events when no memory traps are installed, still perform PC-trap checks before each instruction when PC traps exist, and reject fast execution with `EmuError::FastRunBlockedByMemoryTraps { memory_trap_count }` before advancing when memory traps are installed. Optional upstream JIT/block runners are out of scope for the `gameroy-core` pin and are not part of the F-A7 public contract.

The spike at `gbf-emu/tests/gameroy_api_spike.rs` either confirms each of (1)–(11) against the pinned rev or amends this list (and the affected sections below) in the implementation PR.
...
impl Emulator {
    ...
    pub fn run_fast_until_pc(&mut self, pc: u16, budget: CycleBudget) -> Result<RunOutcome, EmuError>;
...
impl From<MCycles> for ClockCycles {
    fn from(v: MCycles) -> Self { ClockCycles(v.0.saturating_mul(4)) }
}

impl ClockCycles {
    pub const fn as_m_cycles_floor(self) -> MCycles { MCycles(self.0 / 4) }
    pub const fn saturating_mul(self, rhs: u64) -> Self { ClockCycles(self.0.saturating_mul(rhs)) }
}
...
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub enum RunOutcome {
    /// Normal outcome of bounded execution: the budget elapsed without halting on a trap or idle.
...
`StepOutcome::Stepped::cycles` is the clock-cycle cost of the just-executed instruction (1..=24 clock = 1..=6 M-cycles). `RunOutcome::BudgetElapsed` is the load-bearing rename: budget exhaustion is a normal outcome of `run_for`, `run_frame`, `run_until_pc`, `run_fast_for`, `run_fast_frame`, and `run_fast_until_pc`, not an `EmuError`. (An earlier draft listed `RunBudgetExhausted` in `EmuError`; that contradicted the `RunOutcome` shape and is removed.) `Idle` distinguishes HALT from STOP — they have different semantics on real hardware and the runtime nucleus cares about the difference. HALT is *not* terminal: a subsequent `step` / `run_for` / `run_fast_for` may exit HALT when an interrupt fires; only STOP is terminal under M0 (and M0 does not emit STOP).

The fast runners are intentionally a second execution tier, not a behavioral replacement for the fully instrumented runners. Use `run_for`, `run_frame`, or `run_until_pc` when trace completeness or memory watchpoints are load-bearing. Use `run_fast_for`, `run_fast_frame`, or `run_fast_until_pc` for long-running compute tests where PC-trap stops and final-state inspection are enough. The fast tier still returns the same `RunOutcome` variants, still checks PC traps before executing the instruction at the current PC, and still stops on HALT/STOP. Its speed comes from avoiding guest memory trace ingestion and memory-trap dispatch; if memory traps are installed, `EmuError::FastRunBlockedByMemoryTraps` is returned immediately instead of silently selecting the slower runner.

A strict `expect_pc_before_budget(pc, budget)` helper is **out of scope** for F-A7. Consumers that want "must reach this PC or fail" call `run_until_pc(pc, budget)?` or, for long-running compute-style tests, `run_fast_until_pc(pc, budget)?`, then `match` on the outcome:
...
cargo test -p gbf-emu -- fast_run::fast_run_until_pc_reaches_target_without_full_step_trace
cargo test -p gbf-emu -- fast_run::fast_run_honors_pc_traps
cargo test -p gbf-emu -- fast_run::fast_run_rejects_memory_traps_without_advancing
cargo test -p gbf-emu -- fast_run::fast_run_suppresses_guest_memory_trace_when_no_memory_traps_are_present
cargo test -p gbf-emu -- fast_run::fast_frame_runs_one_dmg_frame_budget
...
1. **Before executing the instruction at the current `pc`**, the dispatcher checks PC traps (`TrapKind::Pc { addr }`). This matches what debuggers expect: "break before executing the opcode at `addr`." `run_until_pc(pc, budget)` and `run_fast_until_pc(pc, budget)` return `Ok(RunOutcome::TrapHit { ... })` immediately if the emulator is already positioned at `pc` (zero-cycle case). PC traps are the only kind whose state report is **pre-instruction**.
...
- `TrapAction::HaltAndReport` returns `StepOutcome::TrapHit { trap_id, kind, cycles }` from `step` / `RunOutcome::TrapHit { ... }` from `run_for` / `run_frame` / `run_until_pc` / `run_fast_for` / `run_fast_frame` / `run_fast_until_pc`. Fast runners reject installed memory traps before advancing, so watchpoints are not skipped and callers do not silently pay slow-run costs.
...
`TraceDropPolicy::HaltAndError` makes the next fully instrumented `step` / `run_for` / `run_until_pc` return `EmuError::TraceCapacityExceeded` instead of dropping. Fast runners suppress guest memory trace ingestion by design and reject installed memory traps before advancing, so they do not silently switch into the trace-capacity-sensitive path. Useful for tests that assert "this run should fit in N events."
...
pub enum EmuError {
    ...
    #[error("Fast run blocked by installed memory traps: {memory_trap_count}")]
    FastRunBlockedByMemoryTraps { memory_trap_count: usize },
...
- `run_for(budget)` / `run_frame()` / `run_until_pc(pc, budget)` — bounded by gameroy-core's runtime; F-A7 adds trap dispatch and trace ingestion at instruction boundaries.
- `run_fast_for(budget)` / `run_fast_frame()` / `run_fast_until_pc(pc, budget)` — the long-running test and bench path. It loops gameroy-core's interpreter primitive directly, performs PC-trap checks only when PC traps are installed, clears `io_trace` instead of normalizing guest memory events when no memory traps exist, and fails with `FastRunBlockedByMemoryTraps` before advancing when memory traps are installed. Large compute ROMs should use this tier when they only need target-PC, PC-trap, idle, budget, and final-state semantics.
...
The crate adds ~1,300 production lines to the workspace plus the test/review-packet surface; total cargo build cost is dominated by gameroy's compile time.
...
2. **T-A7.2 adapter.rs + primitives.rs** (parallel with T-A7.3). Two days. The largest module by LOC; wraps gameroy and exposes the execution, memory, register, snapshot, joypad/framebuffer, and harness verbs.
...
| Layer                    | Coverage                                                                                                  |
|--------------------------|-----------------------------------------------------------------------------------------------------------|
| Compile-only spike       | `tests/gameroy_api_spike.rs` (or `examples/gameroy_api_spike.rs`) compiles against the pinned `gameroy-core` rev and proves the construction / interpreter-step / fast-bulk-runner strategy / save-state / access-observation / framebuffer / joypad paths the RFC names. The implementation PR amends the RFC if any assumption fails. |
...
| Unit / property          | Per-module tests against gameroy-core behavior (register state, bus vs. peek, snapshot round-trip). Fast-run tests prove target-PC stop, PC-trap stop, explicit memory-trap rejection with no advancement, trace suppression, and the named DMG-frame budget. Trace normalization fixture-driven. |
| Integration              | `tests/determinism_single_path.rs` — same ROM + same policy → byte-equal `(framebuffer, regs, mem_dump)` after fixed `ClockCycles`. `tests/tiny_rom_boots.rs` — F-A1's deferred boot test (fast-run PC reaches `$0150` within `MAX_TINY_ROM_BOOT_BUDGET` from `BootMode::PostBootDmg`). `tests/snapshot_round_trip.rs` — save → restore preserves state byte-for-byte; lineage rejection is typed. `tests/cross_module_conformance.rs` — every cross-module invariant. |
...
| Negative                 | `EmuError::SnapshotRomMismatch` / `SnapshotPolicyMismatch` / `SnapshotEmuVersionMismatch` on mismatched lineage. `EmuError::HarnessMagicMismatch` on wrong magic. `EmuError::HarnessSequenceMismatch` on command-block seq drift. `EmuError::TraceCapacityExceeded` on overflow with `HaltAndError`. `EmuError::FastRunBlockedByMemoryTraps` when a fast runner is called with memory traps installed. `TrapPredicateError::SourceRequiresEvaluator` on raw `Source` predicate. `AddressRangeError::StartAfterEnd` on inverted ranges. `TrapPersistenceError::ClosureOnly` on attempted serialization of closure-only entries. |
...
All tests run as part of the workspace pre-commit hook (`cargo test --workspace --all-features`). The `tests/tiny_rom_boots.rs` test is *the* load-bearing acceptance gate that F-A7 actually runs ROMs end-to-end, and the `tests/fast_run.rs` tests prove the bulk API intended for long-running emulator workloads.
...
3. **Cycle units are typed.** `ClockCycles(u64)` and `MCycles(u64)` are separate newtypes. `CycleBudget` is the input to `run_for`, `run_until_pc`, `run_fast_for`, and `run_fast_until_pc`. `DMG_FRAME_CLOCK_CYCLES = ClockCycles(154 * 456)` names a DMG frame budget for `run_frame`, `run_fast_frame`, and multi-frame test budgets. The clock-count source of truth is gameroy-core's clock; M-cycle views are derived as `clock / 4`.
4. **Budget exhaustion is a `RunOutcome`, not an `EmuError`.** `RunOutcome::BudgetElapsed { observed, requested }` is the normal outcome of bounded execution (`run_for`, `run_frame`, `run_until_pc`, `run_fast_for`, `run_fast_frame`, `run_fast_until_pc`). An earlier draft put `RunBudgetExhausted` in `EmuError` and that contradicted the `RunOutcome` shape; the unified shape lives in `RunOutcome`.
5. **`Emulator` is `&mut self`-method-style, not callback-style.** Callback-style would force the consumer to maintain its own state machine across callbacks; the verb form keeps state in the `Emulator` and the consumer's loop reads naturally. The cost is "you cannot interrupt a bounded runner from another thread"; F-A7 commits to that limitation explicitly (single-threaded substrate by design).
...
9. **PC traps fire BEFORE executing the instruction at the registered address.** This is what every debugger expects, including gameroy-core's own debugger as documented upstream. `run_until_pc(pc, budget)` and `run_fast_until_pc(pc, budget)` return immediately if the emulator is already positioned at `pc`.
...
45. **Fast/bulk execution is a first-class bounded API, not an unbounded runner.** `gameroy-core` does not provide a public headless frame/budget runner that F-A7 treats as the API contract, so `run_fast_for`, `run_fast_frame`, and `run_fast_until_pc` loop the public interpreter primitive directly. They are for long-running tests and benches that need to run faster than real time: PC traps and target-PC stops are still checked before instruction execution, CPU idle and budget expiry are still surfaced as `RunOutcome`, guest memory trace ingestion is suppressed when no memory traps are installed, and installed memory traps return `EmuError::FastRunBlockedByMemoryTraps` before execution advances. Callers must explicitly choose the fully instrumented `run_for` / `run_until_pc` path if they want memory-watchpoint semantics. (See §3.7.1 finding 11.)
...
| Trap dispatcher's per-step cost dominates long-running emulator tests        | Low        | M0 expects single-digit traps for instrumented runs; the linear scan is fine there. Long-running compute tests use `run_fast_for` / `run_fast_until_pc`, which avoid guest memory-trace ingestion and memory-trap dispatch when no memory traps are installed. If memory traps are present, fast-run returns a typed error so the caller decides whether to remove traps or use the full runner. If PC-trap volume ever dominates the fast path, an indexed PC trap table is a follow-up. |
...
| `run_frame()` uses the named one-DMG-frame clock budget                         | `fast_run::fast_frame_runs_one_dmg_frame_budget`                                                    |
...
| `run_fast_until_pc` reaches a target PC without ingesting a full guest memory trace | `fast_run::fast_run_until_pc_reaches_target_without_full_step_trace`                               |
| `run_fast_for` honors registered PC traps                                       | `fast_run::fast_run_honors_pc_traps`                                                              |
| `run_fast_for` rejects installed memory traps without advancing execution     | `fast_run::fast_run_rejects_memory_traps_without_advancing`                                     |
| `run_fast_for` suppresses guest memory trace ingestion when no memory traps are installed | `fast_run::fast_run_suppresses_guest_memory_trace_when_no_memory_traps_are_present`            |
...
| F-A1's `tiny_rom.gb` boots from `BootMode::PostBootDmg`: `run_fast_until_pc` reaches `$0150` | `tests/tiny_rom_boots.rs`                                                                                |
...
| `gbf-emu/tests/fast_run.rs`                 | New (bulk execution API gates)                                    | ~110         |
...
**Total: ~2,600 LOC, ~65% of which is tests, fixtures, and integration coverage.** The spike file, fast-run test file, and fixture provenance are the new entries vs. the prior estimate.
...
- **Adapter API proof**: the §3.7 adapter spike compiles green; a one-page summary lists the actual public-API paths used (construction, interpreter step, fast/bulk runner strategy, save-state, access observation, framebuffer, joypad).
- **Fast-run performance/correctness note**: which entry points are fully instrumented (`run_for`, `run_frame`, `run_until_pc`) vs. bulk/low-overhead (`run_fast_for`, `run_fast_frame`, `run_fast_until_pc`), what correctness semantics the fast path preserves, what it intentionally suppresses (guest memory trace ingestion), and why memory traps trigger `FastRunBlockedByMemoryTraps` instead of silent fallback.
...
- **Correctness dossier** — for each of the highest-risk surfaces (determinism single-path; snapshot lineage rejection; trap dispatcher PC-trap-fires-before-instruction correctness; fast-run PC-trap / memory-trap-rejection / trace-suppression behavior; trace normalization for BANK1/BANK2; harness magic-byte parsing; F-A1 boot test), the packet records the invariant, the test or type or citation that proves it, and the failure mode if it ever drifts.
- **Test coverage report** — what `cargo test -p gbf-emu` runs, how it groups, what it asserts, including the `tests/fast_run.rs` bulk-API gates, and any portions deliberately not covered (with reasoning).
...
- **API guide** — the public surface of each module, the value objects (with their invariants), the error variants, the fully instrumented vs. fast-run execution tiers, and the (pre-1.0) source-break policy.
- **Reviewer checklist** — the binary questions the reviewer should be able to mark off (e.g., "every consumer goes through `Emulator`", "`DeterminismPolicy::default()` is locked-down", "no other crate imports `gameroy-core`", "the fast-run tier rejects memory traps without advancing", "F-A1's `tiny_rom.gb` boots within `MAX_TINY_ROM_BOOT_BUDGET`").
...
- **F-A1 follow-up evidence** — confirmation that `tests/tiny_rom_boots.rs` exists and passes via `run_fast_until_pc`, with the exact `tiny_rom.gb` SHA-256 it consumed and the explicit `BootMode` it used; the F-A1 §16 deferred row is now closed in this PR.
...
- the fast-run tests prove target-PC stop, PC-trap handling, explicit memory-trap rejection with no advancement, trace suppression, and the one-frame budget;
...
1. **Does the pinned `gameroy-core` revision actually expose the construction / interpreter-step / save-state / framebuffer / cartridge-loading / access-observation API surface this RFC assumes, and is the fast-bulk-runner strategy the right response to that surface?** The §3.7 adapter spike answers this; if the answer is "no" for any of those surfaces, which assumptions in §4–§8 change? This is the single most important review question — most of the RFC depends on the answer.
...
3. Is the **`Emulator` verb set** at the right granularity? Specifically, is the split between fully instrumented `run_for` / `run_frame` / `run_until_pc` and low-overhead `run_fast_for` / `run_fast_frame` / `run_fast_until_pc` the right M0 shape, or is a more general `run_until { pc, mem_predicate, max_cycles }`-style API needed? Adding verbs is intentional source-break pre-1.0; getting the M0 set right at closure time avoids one round of consumer churn.

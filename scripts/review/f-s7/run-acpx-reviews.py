...
def review_prompt(
    ...
    return "\n".join(
        [
            ...
            "the current ACPX reviewers return PASS. If a pre-materialization or refresh "
            "verify-packet run fails only because those two final review JSONs are absent or stale "
            "relative to the HEAD you are currently reviewing, treat that absence/staleness as "
            "expected bootstrap state; the runner and validate-reviews.py will enforce both files "
            "after PASS. Also ignore dirty-worktree failures caused only by docs/review/f-s7/raw/"
            f"{bead}-*.{{command,extracted,nonpass,stderr,stdout}}.txt files generated by the "
            "current review attempt. Still block on any other missing evidence, dirty worktree, "
            "open bead state, stale non-final-review evidence, or unresolved packet failure.",
            "Return exactly one JSON object. No Markdown wrapper. Shape:",
            "{",
            '  "verdict": "PASS or NEEDS_CHANGES",',
            f'  "personas": {json.dumps(personas)},',

...
S7_PRODUCTION_RUNNER="gbf-experiments/src/s7/production_runner.rs"
...
S7_AUDIT_PRODUCTION_RUNNER_TEST="scripts/tests/s7_audit_production_runner_test.sh"
...
require_present 'ProduceProductionBundle\(S7ProduceProductionBundleArgs\)' "$S7_CLI"
...
require_present 'gbf s7 produce-production-bundle' "$S7_CLI"
require_present 'produce-production-bundle' "$S7_CLI_TEST"
...
require_present 'produce_s7_production_bundle' "$S7_CLI" "$S7_PRODUCTION_RUNNER"
require_present 'S7ProductionBundleInputs' "$S7_CLI" "$S7_PRODUCTION_RUNNER"
require_present 's7_production_bundle_manifest\.v1' "$S7_CLI" "$S7_PRODUCTION_RUNNER" "$PACKET_ASSEMBLER"
require_present 's7_production_runner\.v1' "$S7_PRODUCTION_RUNNER" "$PACKET_ASSEMBLER" "scripts/tests/s7_assemble_packet_test.sh"
require_present 'live_burn_adamw_moe_lm_state_per_topology_seed' "$S7_PRODUCTION_RUNNER" "$PACKET_ASSEMBLER"
require_present 'S7_OPTIMIZER_STEPS' "$S7_PRODUCTION_RUNNER"
require_present 's7_grad_log\.v1' "$S7_PRODUCTION_RUNNER"
require_present 's7_router_step_telemetry\.v1' "$S7_PRODUCTION_RUNNER"
require_present 'optimizer_step' "$S7_PRODUCTION_RUNNER"
require_present 'BurnGradientsParams::from_grads' "$S7_PRODUCTION_RUNNER"
require_present 'optimizer.step' "$S7_PRODUCTION_RUNNER"
require_present 'gutenberg_manifest_sha' "$S7_PRODUCTION_RUNNER" "$PACKET_ASSEMBLER"
require_present 'train_corpus_sha' "$S7_PRODUCTION_RUNNER" "$PACKET_ASSEMBLER"
require_present 'val_corpus_sha' "$S7_PRODUCTION_RUNNER" "$PACKET_ASSEMBLER"
...
require_present 'S7 production runner audit failed' "$VERIFY_PACKET_SCRIPT"
...
require_present 's7_audit_production_runner_test\.sh' "$S7_PR_WORKFLOW"
...
require_present 'scripts/review/f-s7/audit-production-runner.py' "$VERIFY_PACKET_SCRIPT"
...
require_present 'audit-production-runner\.py' "$VERIFY_PACKET_SCRIPT"
...
require_present 'S7 production runner audit: NEEDS_CHANGES' "$PRODUCTION_RUNNER_AUDITOR"
require_present 'bd-3e10j' "$PRODUCTION_RUNNER_AUDITOR" "$S7_AUDIT_PRODUCTION_RUNNER_TEST"
require_present 's7_production_bundle_manifest\.v1' "$PRODUCTION_RUNNER_AUDITOR"
require_present 's7_production_runner\.v1' "$PRODUCTION_RUNNER_AUDITOR" "$PACKET_ASSEMBLER"
require_present 'production_runner\.rs' "$PRODUCTION_RUNNER_AUDITOR"
require_present 'BurnMoeS7ModelState' "$PRODUCTION_RUNNER_AUDITOR" "$S7_PRODUCTION_RUNNER"
require_present 'BurnByteMoeS7Model' "$PRODUCTION_RUNNER_AUDITOR" "$S7_PRODUCTION_RUNNER"
require_present 'FORBIDDEN_CODE_SURFACE_SNIPPETS' "$PRODUCTION_RUNNER_AUDITOR"
require_present 'ProduceProductionBundle' "$PRODUCTION_RUNNER_AUDITOR"
require_present '--skip-code-surface' "$PRODUCTION_RUNNER_AUDITOR" "$S7_AUDIT_PRODUCTION_RUNNER_TEST"
require_present 'code_surface_checked' "$PRODUCTION_RUNNER_AUDITOR"
require_present 'production_closure_retrain_score' "$PRODUCTION_RUNNER_AUDITOR"
require_present 'optimizer/model-state' "$PRODUCTION_RUNNER_AUDITOR"
require_present 'do not consume fixture/smoke/replay artifacts as production' "$PRODUCTION_RUNNER_AUDITOR" "$S7_AUDIT_PRODUCTION_RUNNER_TEST"
require_present 'production-looking S7 artifacts exist while runner owner is unresolved' "$PRODUCTION_RUNNER_AUDITOR" "$S7_AUDIT_PRODUCTION_RUNNER_TEST"
require_present 's7_production_bundle_manifest\.v1' "$S7_AUDIT_PRODUCTION_RUNNER_TEST"
require_present 's7_production_bundle_manifest\.v1' "$PACKET_ASSEMBLER"
require_present 'validate_production_runner' "$PACKET_ASSEMBLER" "scripts/tests/s7_assemble_packet_test.sh"
require_present 'production_runner\.sweep_producer_kind must be production_closure_retrain_score' "$PACKET_ASSEMBLER" "scripts/tests/s7_assemble_packet_test.sh"
...
require_present '--allow-absolute-inputs' "$PACKET_ASSEMBLER" "scripts/tests/s7_assemble_packet_test.sh"
...
require_present 'absolute bundle input path requires --allow-absolute-inputs' "$PACKET_ASSEMBLER" "scripts/tests/s7_assemble_packet_test.sh"
...
require_present 'Final-review bootstrap rule' "$ACPX_REVIEW_RUNNER" "$S7_RUN_ACPX_TEST"
require_present 'only because those two final review JSONs are absent' "$ACPX_REVIEW_RUNNER" "$S7_RUN_ACPX_TEST"
require_present 'absent or stale relative to the HEAD' "$ACPX_REVIEW_RUNNER" "$S7_RUN_ACPX_TEST"
require_present 'dirty-worktree failures caused only by docs/review/f-s7/raw' "$ACPX_REVIEW_RUNNER" "$S7_RUN_ACPX_TEST"
...
[[ -x "$PRODUCTION_RUNNER_AUDITOR" ]] || {
  echo "schema discipline failed: $PRODUCTION_RUNNER_AUDITOR must be executable" >&2
  exit 1
}

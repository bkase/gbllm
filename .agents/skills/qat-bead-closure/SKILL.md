...
- For annealed threshold schedules, do not reuse phase-local soft progress directly. Expose named threshold schedule progress that is zero before the owner phase, advances only during that phase, and remains complete in later hard/export-selection phases; add a boundary test across the phase after annealing completes.

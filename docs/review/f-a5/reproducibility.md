...
The PNG is a 4x nearest-neighbor rendering of the real 160x144 `gbf-emu` framebuffer after booting the F-A5 demo ROM and placing the `FA5 OK` visual-review prompt into the bootstrapped BG map. The F-A5 M0 font is the deterministic boxed bring-up font from `tools/font/build_font.py`, so the prompt verifies the expected glyph cells rather than polished Latin letterforms.

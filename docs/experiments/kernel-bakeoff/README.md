...
| profile | MACs/token | V1 | V2 | V3 |
|---|---|---|---|---|
| Toy1 (d32/ff64/2blk) | 12,800 | 2.0 | 5.6 | **10.6** |
| MoeTiny (d64/ff128/4blk) | 87,040 | 0.30 | 0.82 | **1.57** |
| UpperBank-96 (d96/ff192/4blk) | 192,000 | 0.13 | 0.37 | **0.71** |
| UpperBank-128 (d128/ff192/4blk) | 272,384 | 0.09 | 0.26 | **0.50** |
...
1. **Weights-as-code is the only strategy that makes MoeTiny interactive**
   (~1.5–2.2 chars/sec). The interpreted kernel that the size-profile registry
   implicitly assumed is ~10x too slow for anything beyond Toy1.
2. **The V3 ROM trade is affordable.** ~4.4 bytes/weight at 40% zeros puts all
   of MoeTiny's ~284k stored weights at ~1.2 MiB of generated code — well
   ...
   ~1.1 B/w).
3. **UpperBank-128 is not interactive under any measured strategy** (≤0.5
   tok/s even as a floor). S8 sizing decisions should treat UpperBank profiles
   as non-interactive/batch demos unless a materially faster kernel appears.
4. **Dispatch (V2) is a respectable data-driven fallback** when ROM is scarce:
   0.25 B/w like V1 but 2.8x faster. Its cost is dominated by the 14-cycle
   threaded dispatch; the LR35902 has no indexed jump, so this is near the
   floor for data-driven decoding.
5. **Architecture implication (planv0):** the compiler needs a
   weights-as-code lowering path for hot expert/mix matvecs. That blurs the
